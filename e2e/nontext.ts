 ...
 *
 * In this repo the first half is the live one.
 *
 * The first half checks a claim the stylesheet makes about itself. `main.css`
 * defines `--control-border` with a comment asserting specific ratios —
 * "#7d93a8 measures 4.02:1 on --surface-soft" in dark, "4.41:1" for the light
 * companion — precisely because the divider token `--border` measures 1.32:1
 * and is wrong for a control's edge. A comment is not evidence: every control
 * here fills with `color-mix(in oklab, var(--surface-soft) N%, transparent)`
 * over a `.panel-card` that is itself a mix over the body's gradient washes,
 * and which of the ~47 `--border` sites are CONTROLS (and so belonged on
 * `--control-border`) is exactly the kind of judgement that drifts —
 * `.commute-regen` shipped with the divider token as its only delineator, and
 * this oracle is what found it. It measures the composite each control is
 * really drawn on, at every driven state, including `:hover` — which persists
 * after a click and is the state a reader occupies the instant after pressing
 * a toggle.
 *
 * The second half is currently vacuous here, and that is measured rather than
 * assumed: this lab's only pseudo-content rule is `.cross-label::before`,
 * `content: ""` — a 3px colour swatch matching each DH line, whose meaning is
 * restated by the visible "DHn LEFT × RIGHT" text beside it. Every ✓ / ✗ / ▲
 * state mark on this page is a real text node, so the arithmetic walk in
 * `contrast.ts` measures them all. The half still runs at every driven state,
 * so the first glyph anyone moves into a `::after` is gated, not invisible.
 *
 ...
 *    That is exactly the shape this lab's controls have: every button fills
 *    with a 55–78% mix of `--surface-soft` into transparency over a card that
 *    is mostly `--surface` — near-identical lightness — so the
 *    `--control-border` edge is the ONLY thing that makes a control visible.
 *
 ...
 *    the wrong surface, and `escaped` is reported so triage can see it. This
 *    lab's one pseudo rule is static (`.cross-label::before` is a flex item),
 *    so nothing here lands in that category today.
 *
 ...
 * background beneath — because a control's fill here is a `color-mix()` over a
 * card that is itself a `color-mix()` over the body's gradients, and reading
 * the declared value would measure something the page never paints.
 */
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * page: `body` is a `linear-gradient(160deg, var(--bg), var(--bg-alt))`
     * running the full height of a document five panels deep, so a control
     * near the top sits on a different surface than one near the bottom. So
     ...
     * `none`) is unmeasurable and paints nothing. The radial branch is
     * load-bearing here too: `body` carries two `radial-gradient(1200px 600px
     * at …)` washes — teal top-left, amber top-right — over that linear base,
     * and they are what sits directly behind the sticky bar's controls and the
     * hero once the translucent mixes above them are composited.
     */
     ...
     *
     * A driven pass here walks a page dense with hex chips, kv rows, keytags
     * and KM blocks, and the expensive part is the generated-content half: it
     * asks for TWO extra computed styles (`::before` and `::after`) on every
     * element on the page. Without the caches the pass re-reads the same
    ...
    // The paint core above is carried verbatim from contrast.ts so the two
    // oracles cannot disagree; these keep the unused declarations referenced.
    void splitTopLevel;
    void over;
    void fade;
    void luminance;
     ...
     * what a page paints. This lab is a direct instance of the failure: `body`
     * declares its surface as a three-layer `background` shorthand — two
     * radial washes over a linear gradient — so `backgroundColor` reads
     * `rgba(0, 0, 0, 0)` on it, and nearly every fill above it is a
     * `color-mix(…, transparent)`. A `background-color`-only walk therefore
    ...
    const CONTROL = [
      ...
      // control?" test below. This lab's `.badge` chips in the related-demos
      // row are bordered and filled, but they carry neither `btn` nor `button`
      // in their class, so under the fleet rule they are judged as prose links
      // that identify themselves by their text; their ink is measured all the
      // same by the 1.4.3 walk in `contrast.ts`.
      'a[role=button]',
      'a[class*=btn]',
      'a[class*=button]',
    ].join(',');
