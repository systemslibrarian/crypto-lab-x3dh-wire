 ...
 *    an unknown backdrop, or a `color-mix()`. Almost every surface in this lab
 *    is one or the other: `body` is two `radial-gradient` washes over a
 *    `linear-gradient`, every `.panel-card` is `color-mix(in oklab,
 *    var(--surface) 88%, transparent)` over that, and everything that carries
 *    MEANING sits on a further mix — the status pills (`.status-pill.ok` /
 *    `.bad`), the armed tamper toggles (`.lab-toggle.on`), the SK compare panel
 *    in both its tones (`.sk-compare--match` / `--mismatch`), every
 *    `.diff-byte` highlight, the KM strip blocks (`.km-block--1..4`), the
 *    caution notes (`.small-note--caution`), the commutativity widget and its
 *    outputs, every kv row and hex chip, and the hero aside. axe resolves none
 *    of them and files them under `incomplete`, so a violations-only gate
 *    measured the contrast of almost nothing this lab paints.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. This lab has three such
 *    live rules and one of them is load-bearing teaching state: `.tl-beat
 *    { opacity: .85 }` fades every timeline beat that is not the current one,
 *    on all three panels that render the strip. The others are `.cl-hero-sub
 *    { opacity: .85 }` and the `opacity: .55` on disabled walkthrough / unlock
 *    buttons (exempt — 1.4.3 does not judge inactive controls). The footer
 *    used to be a fourth — an inline `opacity: .7` over the whole element that
 *    pushed its 13.6px light-theme text to ~3.4:1 — and this walk is why that
 *    rule is gone rather than shipped.
 *
 ...
 * the walk as a pair rather than fading the foreground alone. `.tl-beat` sets
 * its own tinted fill (`color-mix(in oklab, var(--surface-soft) 45%,
 * transparent)`), so this is exactly the shape the pair models: at `opacity:
 * .85` the beat's fill fades too, and the ink lands on the panel card showing
 * through it.
 *
 ...
 *    boxes are every `.kv-grid div` row and `.dh-list article`
 *    (`overflow-x: auto` — they hold 64-char hex and non-wrapping code tokens)
 *    and `.sk-diff-code` (`overflow-x: auto`, the byte diff of the two diverged
 *    secrets). At 380px several kv rows genuinely scroll. Content scrolled past
 *    any of them is not dimmed or partly drawn — it is absent, and asking what
 *    colour it sits on has no answer. Its rect is outside every ancestor's box,
 ...
 *    lab's one real figure IS an SVG — the `.cross-svg` crossing diagram in
 *    Panel 3 — but it renders no text at all: it is four stroke-only `<line>`s
 *    whose `<title>` children are non-rendering, the whole element is
 *    `aria-hidden`, and its information is duplicated by the visible
 *    `.cross-labels` list and the labelled `.cross-node` HTML spans overlaid on
 *    it. So the sibling walk finds nothing to do today. The branch is kept
 ...
 *    `<text>` onto the `<svg>`'s own transparent background rather than onto
 *    the `<rect>` drawn under it, and report a ratio nothing on screen has.
 *
 ...
 * not one — nor is it one to axe's `color-contrast` rule. This lab paints no
 * glyphs that way: its only pseudo-content rule is `.cross-label::before`,
 * `content: ""` — a colour swatch, judged as non-text by `nontext.ts`. Every
 * ✓ / ✗ / ▲ state mark here is a real text node in a real element (the status
 * pills, the SK verdict, the decrypt verdict), which is exactly why this walk
 * measures all of them. `nontext.ts` still audits pseudo-content at every
 * driven state, so the first glyph anyone adds is gated rather than invisible.
 */
 ...
 * uses. Nothing here is short-lived enough to need the narrowed form: every
 * animation is pure CSS (`rise-in` on each panel card, `cross-draw` on the DH
 * lines, `km-slide`/`km-pulse` on the KM strip), and under the reduced motion
 * this gate asserts they are either cancelled outright by their own
 * `prefers-reduced-motion` rules or collapsed to 0.01ms by the global block,
 * so every element is at its end state before a scan starts. The glossary
 * popovers live exactly as long as their hover/focus, and the gate holds that
 * state open while it scans.
 *
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A hand-rolled `rgba()` regex is not enough here: every status pill,
     * tamper toggle, SK compare tone, KM block and kv row in this lab is
     * authored with `color-mix(in oklab, …)`, and Chromium reports that to
     ...
     * running the full height of a document five panels deep, so text near the
     * top sits on a different surface than text near the bottom, and `.hero`
     * is a `linear-gradient(145deg, var(--surface), var(--surface-soft))`. So
     ...
     * here too: `body` carries two `radial-gradient(1200px 600px at -10% -10%,
     * …)` washes — teal top-left, amber top-right — over that linear base.
     */
     ...
     *
     * A driven pass here walks a page that renders dozens of hex chips, kv
     * rows, keytags, timeline beats and KM blocks per panel — an expanded hex
     * chip alone is a 64-character mono string in a button among many sibling
     * chips, all re-walking the same ancestors up to `<body>`. Without the
     ...
     * that is every `.kv-grid div` row and `.dh-list article`
     * (`overflow-x: auto`, holding 64-char hex and nowrap code tokens) and
     * `.sk-diff-code` (`overflow-x: auto`, the byte diff of diverged secrets).
     */
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // ratio was 6.15:1. This page is MADE of that shape: the `.cross-svg`
      // crossing diagram is four stroke-only <line>s (`fill: none` in CSS, but
      // the guard cannot rely on one lab's stylesheet), and the shared
      // header's hamburger is three more. None of them sits under SVG text
      // today — the crossing diagram renders no <text> at all and its labels
      // are HTML spans overlaid via CSS — but the FILLED guard is what keeps
      // the first <text> added to that diagram from being judged against an
      // invented black backdrop.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // `.dh-threat` disclosures in Panel 3 — one "What it defends against"
      // per DH leg, all shipped shut. It also catches the glossary popovers:
      // `.gloss-pop` rests at `visibility: hidden; opacity: 0` until its term
      // is hovered or focused. The gate opens a threat disclosure by clicking
      // its <summary> and holds a popover open by hovering its term — the
      // routes a reader has — and scans both, rather than forcing `.open` or
      // `visibility` from script the way the gate this replaces forced styles.
      ...
      // `top:-100%`. Measuring a parked copy invents a failure for text that
     ...
     *
     * What this lab hides was enumerated from `ui.ts` rather than assumed,
     * because that is the shared blind spot where both oracles stop looking:
     *
     *  - `.holds-legend` — the Alice/Bob/public/private colour legend under the
     *    who-holds-what grid. Its four words duplicate the visible box headings
     *    and keytags directly above it. Ink is `--text-dim` on the panel card,
     *    the same pair measured on dozens of exposed elements every scan.
     *  - `.cross-svg` — the crossing diagram's four stroke-only lines; renders
     *    no text, and its DH pairings are duplicated by the visible (and
     *    measured) `.cross-labels` list.
     *  - the arrows: `.commute-arrow` ("↓ X25519"), the `.converge2-flow` row
     *    ("↓ feed KM as HKDF input keying material") and `.km-out-arrow` ("→").
     *    Each narrates flow that the adjacent MEASURED captions state in words
     *    ("Step 2 — hash. HKDF-SHA-256 extracts and expands …"). Ink is
     *    `--text-dim` on the same surfaces as those captions.
     *  - the dots: `.tl-dot` and `.hexchip-dot`, which paint no text at all.
     *
     * The only other `aria-hidden` on the page is on the shared top bar's two
     * decorative SVG marks. Nothing here hides a VALUE: no verdict, no key, no
     * hex byte and no status pill is inside an `aria-hidden` subtree — which
     ...
     *
     * This lab exercises the guard for real: every `<line>` in the crossing
     * diagram carries a `<title>` child ("DH1: IK_A × SPK_B" …), and `<title>`
     * is exactly such a non-rendering carrier of character data. Those lines
     * are inside an `aria-hidden` SVG so the previous guard already skips
     * them, but the two guards fail independently — remove the `aria-hidden`
     * and this one is what stands between the walk and a 1:1 failure
     ...
     * a three-layer `background` shorthand on `body` — two `radial-gradient`
     * washes over a `linear-gradient(160deg, var(--bg), var(--bg-alt))` — and
     * gives it only `min-height: 100vh`, so on a five-panel document several
     * screens deep the body's own border box can end before the content does.
     * The root element paints nothing at all. Without the propagation rule
     * below, elements past the painted box would find no ancestor paint and
     * fall through to WHITE, which in the dark theme (`--bg: #0b1218`) reports
     * near-white ink on a page that does not exist. Reading `backgroundColor`
     * alone would be just as wrong here: it is `rgba(0, 0, 0, 0)` on the body,
     * because the colour lives inside the gradient layers and only `paintAt`
     * composites them.
     */
